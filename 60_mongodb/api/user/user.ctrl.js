...
const signup = (req, res) => {
    ...
    UserModel.findOne({ email }, (err, user) => {
        ...
        if(user) return res.status(409).send("이미 사용중인 이메일 입니다.");
        // 등록 (비밀번호 암호화)
        const saltRounds = 10;
...
const login = (req, res) => {
    ...
    UserModel.findOne({ email }, (err, user) => {
        if(err) return res.status(500).send("사용자 조회시 오류가 발생했습니다.");
        if(!user) return res.status(404).send("가입하지 않은 email 입니다.");
...
const checkAuth = (req, res, next) => {
    // 모든 화면에 공통으로 사용한는 데이터
    res.locals.user = null;
    
    // 쿠키에서 토큰을 가져옴 (cookies)
